% ========================================================================= 
% This code implements an example with synthetic data to compare the
% ALS optimization semialgebraic algorithms proposed in the paper below with 
% the structured data fusion (SDF) framework of Sorber et al.
%
...
mc_err_z_cp_opt_a = zeros(num_MC_runs, 1);
mc_err_z_cp_opt_r = zeros(num_MC_runs, 1);
mc_err_z_cp_alg   = zeros(num_MC_runs, 1);
mc_err_z_cp_SDF_r = zeros(num_MC_runs, 1);

mc_time_cp_opt_a = zeros(num_MC_runs, 1);
mc_time_cp_opt_r = zeros(num_MC_runs, 1);
mc_time_cp_alg   = zeros(num_MC_runs, 1);
mc_time_cp_SDF_r = zeros(num_MC_runs, 1);
...
for n_run=1:num_MC_runs
    ...
    opts.alg_use_regression = false; % use regression to estimate the factors in the algebraic algorithm
    ...
    tic
    [Z_SDF, Psi_SDF, costf_SDF] = PECOTED_SDF_multi_init_runs(Y, P1, P2, P3, rankZ, rankPsi, Gamma, opts, num_runs);
    t_SDF_r = toc / num_runs;


    % compute avg errors and times
    mc_err_z_cp_opt_a(n_run, 1) = comp_NRMSE(Z, Z_cp_opt_a);
    mc_err_z_cp_opt_r(n_run, 1) = comp_NRMSE(Z, Z_cp_opt_r);
    mc_err_z_cp_alg(n_run, 1)   = comp_NRMSE(Z, Z_cp_alg);
    mc_err_z_cp_SDF_r(n_run, 1) = comp_NRMSE(Z, Z_SDF);


    % store times:
    mc_time_cp_opt_a(n_run, 1) = t_opt_a;
    mc_time_cp_opt_r(n_run, 1) = t_opt_r;
    mc_time_cp_alg(n_run, 1)   = t_alg;
    mc_time_cp_SDF_r(n_run, 1) = t_SDF_r;
...
% show times 
fprintf('\n\n')
fprintf('Semi-Alg NRMSE  %.5f, std= %1.3f \n', mean(mc_err_z_cp_alg(:)),   std(mc_err_z_cp_alg(:)) )
fprintf('Opt in.1 NRMSE  %.5f, std= %1.3f \n', mean(mc_err_z_cp_opt_a(:)), std(mc_err_z_cp_opt_a(:)) )
fprintf('Opt in.2 NRMSE  %.5f, std= %1.3f \n', mean(mc_err_z_cp_opt_r(:)), std(mc_err_z_cp_opt_r(:)) )
fprintf('SDF NRMSE       %.5f, std= %1.3f \n', mean(mc_err_z_cp_SDF_r(:)), std(mc_err_z_cp_SDF_r(:)) )
